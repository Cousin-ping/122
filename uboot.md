...
kconfig是make menuconfig的源码，可以修改这部分原码选择那些编译进内核
仿照kconfig中的其他编译选项来编写就行
kconfig这个脚本文件中的tristate表示3种状态 
1 编译进内核        按下y
2 不编译进内核      按下n
3 编译为模块        按下m
bool 表示2种状态
...
make modules
```
把.config文件中选定编译为模块的选项编译为模块(.ko文件)
```sh
insmod led.ko #加载led驱动模块文件
```
```sh
ln -sf 软连接实际指向的路径 软连接文件
# s参数表示创建软连接
# f表示强制创建
```
kernel启动初期调用内部函数解析dtb文件，得到硬件信息，并组装成数据结构，配合策略函数工作。
__这个dtb文件只在硬件初始化的时候有意义，初始化完成之后这部分内存就没意义了，可以释放（个人暴论）__
## 设备树格式讲解
