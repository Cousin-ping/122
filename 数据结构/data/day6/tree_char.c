...
//创建二叉树,用递归函数创建
bitree_list_t CreateBitree(int n, int i, datatype data) //n:节点数，i：节点的编号
{
    ...
    if (2 * i <= n) //有左孩子
    {
        r->lchild = CreateBitree(n,2*i,data+i);
    }
    else
    {
        r->lchild = NULL;
    ...
    if (2 * i + 1 <= n) //有右孩子
    {
        r->rchild = CreateBitree(n,2*i+1,data+i+1);
    }
    else
    {
        r->rchild = NULL;
...
{
    bitree_list_t r = CreateBitree(5, 1,'A');
    printf("前序遍历：");
    PreOrder(r);
    printf("\n");
    printf("中序遍历：");
